...
/**
 * This class is used to build up a covariance matrix and extract the
 * eigenvalues and eigenvectors of it. Portions of the code are taken from the
 * Java Matrix Package (JAMA), a cooperative product of The MathWorks and the
...
public class CovarianceMatrix
{
	...
	/**
	 * Constructor that builds up the matrix and calculates the Eigenvalues and
	 * Vectors internally.
	 * 
	 * @param vectors
	 */
	...
	public CovarianceMatrix( short[][] vectors )
	{
		...
		// build "small" matrix
		// FIXME: if (vectors.length > length of single vector) then the small
		// matrix will be the bigger one!
		...
		for( int rowIndex=0; rowIndex < smallMatrix.length; rowIndex++ )
		{
			...
			for( int colIndex=0; colIndex < rowIndex + 1; colIndex++ )
			{
				...
				{
					smallMatrix[rowIndex][colIndex]+=vectors[rowIndex][i]
							* vectors[colIndex][i];
				}
		...
		/*
		 * Note: From here on the eigenvectors are stored in the COLUMNS of the
		 * small matrix!
		 * 
		 * Uncomment this example to print all elements of eigenvector e:
		 * 
		 * int e = 0; for (int j = 0; j < smallMatrix.length; j++)
		 * System.out.println(smallMatrix[0][e]);
		 */

		// build eigenvalues of big matrix
		ArrayList<EigenValueAndVector> tempList=new ArrayList<EigenValueAndVector>(
				dimension );
		 ...
		 * for (int i = 0; i < vectorLength; i++){ for (int j = 0; j<
		 * vectorLength; j++){
		 * 
		 * double matrixElement = getBigMatrixElement(i, j, vectors);
		 * 
		 * for (int k = 0; k < dimension; k++){ tempVectors[k][i] +=
		 * matrixElement * tempList.get(k).eigenVector[j]; } } } for (int i = 0;
		 * i < tempList.size(); i++){ int element = -1; loop: for (int j = 0; j
		 * < tempList.size(); j++){ if (tempVectors[i][j] != 0){ element = j;
		 * System.err.println(eig[i] + " ?= "+tempVectors[i][element] /
		 * tempList.get(i).eigenVector[element]); break loop; } }
		 * tempList.get(i).eigenValue = tempVectors[i][element] /
		 * tempList.get(i).eigenVector[element]; }
		 */
	...
	/**
	 * Calculates elements of the "bigger" matrix - used for testing purpose
	 * only.
	 * 
	 * @param row
	 * @param col
	...
	/**
	 * Returns the Eigenvalue specified by a given number. The Values are sorted
	 * biggest-first.
	 * 
	 * @param number
	 * @return
	...
	/**
	 * Returns the Eigenvector specified by a given number. The Eigenvectors are
	...
	/**
	 * Symmetric Householder reduction to tridiagonal form as used in JAMA
	 * Package.
	 */
	...
	/**
	 * Sortable structure holding eigenvalues and their corresponding
	 * eigenvectors.
	 */
	private static class EigenValueAndVector implements Comparable
	{
		...
		public int compareTo( Object o )
		{
			...
			if( this.eigenValue < that.eigenValue )
				return 1;
			if( this.eigenValue > that.eigenValue )
				return -1;
			return 0;
